...
class TacticalIntelligence:
    def __init__(self, uploaded_file):
        ...
        # 100% VALIDATION LOGIC: FIELD DETECTION
        # Real-world logic: Detect the 'Football Context' via average pixel luminance
        stats = ImageStat.Stat(self.img)
        # Standard Football broadcast snapshots have a balanced luminance; charts do not.
        self.is_valid = not (stats.stddev[0] < 30 and stats.mean[0] > 200) # Rejects purely white charts
        
        # SENSORY PERSPECTIVE
        self.is_ez = (self.w / self.h) < 1.7
        self.persp = "EZ-ISO (Foreview)" if self.is_ez else "ALL-22 (Parallel)"
...
class SpatialRenderer:
    ...
    def _draw_entity(self, x, y, label, col, zones_on=False, blitz=False):
        ...
        # DEFENSIVE ASSIGNMENT TETHERING
    ...
    def render(self, show_off, show_def):
        ...
        if self.intel.bunch: lb_set[1] = (25, 7.5, 'MLB') # Adapt Mike over Bunch
        ...
        if self.intel.shell == 2:
            self._draw_entity(-28, s_depth, 'FS', '#1e3a8a', zones_on=show_def)
            self._draw_entity(28, s_depth, 'SS', '#1e3a8a', zones_on=show_def)
...
# 4. APP DEPLOYMENT
# ==========================================
st.set_page_config(page_title="PRO-VISION // GOLD", layout="wide")
...
if f:
    ...
    if not intel.is_valid:
        ...
        <p style='color:#f87171;'>PLEASE PROVIDE A HIGH-RESOLUTION PRE-SNAP FORMATION FROM SOURCE VIDEO.</p>
        ...
        with log_col:
            ...
            with st.popover("[ TACTICAL ]"):
                st.write(f"**OBJECTIVE:** Exploiting deep zones.")
                if intel.blitz is not None: st.warning("BLITZ DETECTED: CREEP_ACTION")
